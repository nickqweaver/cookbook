# =============================================================================
# Cookbook Application Dockerfile
# =============================================================================
...
RUN addgroup --system --gid 1001 nodejs && \
    adduser --system --uid 1001 cookbook

# Create the data directory for SQLite database
# This will be mounted as a volume to persist data
RUN mkdir -p /app/data && chown cookbook:nodejs /app/data

# Copy only what's needed from the builder stage
# - .output: The compiled production server
# - drizzle: Database migrations (needed for db:migrate)
# - drizzle.config.ts + schema: Required by drizzle-kit migrate command
# - package.json: For npm scripts
# - node_modules: Runtime dependencies (better-sqlite3 native bindings)
COPY --from=builder --chown=cookbook:nodejs /app/.output ./.output
COPY --from=builder --chown=cookbook:nodejs /app/drizzle ./drizzle
COPY --from=builder --chown=cookbook:nodejs /app/drizzle.config.ts ./
COPY --from=builder --chown=cookbook:nodejs /app/src/db/schema.ts ./src/db/
COPY --from=builder --chown=cookbook:nodejs /app/package.json ./
COPY --from=builder --chown=cookbook:nodejs /app/node_modules ./node_modules
...
ENV DATABASE_URL=/app/data/cookbook.db
